...
int U::convUTF(int byte1, string charsRead, unsigned int &index) const {	// index is passed by reference because it will change the actual value of index
	...
	if ( (byte1 & 0xE0) == 0xC0 ) {
		++index;						// preIncr index so we can get the next byte
		c = charsRead.at(index);
		int byte2 = c;
...
void U::propfile(string filename) {
	if (propOpened) {
		ostringstream oss;
...
int U::propcount(string prop) const {
	char byte;			// will store a character (byte) at a time from input files
	int uni;			// will store the unicode value of a character
	int count = 0;		// occurences of prop

	for (unsigned int i = 0; i < charsRead.length(); i++) {	
		byte = charsRead.at(i);
		uni = convUTF(byte, charsRead, i);	// i will get incremented in convUTF since it is passed by reference and convUTF increments i
											// so we can get all bytes of a UTF8 char
		
		if (!propsMap.count(uni)) {		// if unicode is not in the props file
			//ostringstream oss;
			//oss << "Unicode code point " << hex << uni << " is not contained in the property file read in";
			//throw oss.str();
			continue;
		}

		if (propsMap.at(uni) == prop) {		// TODO: might need to error check this (i.e. if uni isn't in props, could get a seg fault)
			count++;
		}
	}
